...
void buildic_reference(FA_Global* FA,atom* atoms,resid* residue,int rnum)
{
  int i,a,b,c;
  float ci[3],ca[3],cb[3],cc[3];
  int rot;
  ...
  // if(!FA->refstructure)
  // {
  //   fprintf(stderr,"the function 'buildic_reference() should not be called without the coordinates of a reference pose\n");
  //   Terminate(1);
  // }
  ...
  for(i=residue[rnum].fatm[rot];i<=residue[rnum].latm[rot];i++)
  {
    if(atoms[i].recs == 'm')
    {
      ...
      ci[0]=atoms[i].coor_ref[0];
      ci[1]=atoms[i].coor_ref[1];
      ci[2]=atoms[i].coor_ref[2];
      ...
      if(a == 0)
      {
        ...
        ca[0]=atoms[a].coor_ref[0];
        ca[1]=atoms[a].coor_ref[1];
        ca[2]=atoms[a].coor_ref[2];
      }
      ...
      if(b == 0)
      {
        ...
        cb[0]=atoms[b].coor_ref[0];
        cb[1]=atoms[b].coor_ref[1];
        cb[2]=atoms[b].coor_ref[2];
      }
      ...
      if(c == 0)
      {
        ...
        cc[0]=atoms[c].coor_ref[0];
        cc[1]=atoms[c].coor_ref[1];
        cc[2]=atoms[c].coor_ref[2];
      }
